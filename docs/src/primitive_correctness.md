...
| Retry | Failed retryable execution scheduled for another attempt. | Retry records failed attempt data and moves job to retry wait/queued future state. | `reschedule_for_retry`, `RetryConfig`, `JobRunner::on_failure` | Integration/failure: retries, DLQ exhaustion, storm control | PG, SQLite, Redis, Memory |
