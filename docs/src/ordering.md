...
`azums` supports per-queue ordering policies to control the sequence in which background jobs are leased by workers.
...
async fn main() -> anyhow::Result<()> {
    ...
    // Configure "order_processing" queue for FIFO leasing (default)
...
- **Guarantee**: With a single worker process (or single-partition worker), runnable jobs are leased in FIFO order. Completion order still depends on handler behavior.
