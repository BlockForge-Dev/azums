...
<body>
  ...
  <script>
    (function bootstrapApiKey() {
      ...
      if (token) localStorage.setItem("azums_api_key", token);
    })();

    function buildHeaders(extra) {
      const headers = Object.assign({}, extra || {});
      const token = localStorage.getItem("azums_api_key");
...
pub async fn metrics_prom(State(state): State<ApiState>) -> Response {
    match state.jobs.metrics_snapshot().await {
        Ok((queued, running, succeeded_last_60s, failed_last_60s)) => {
            let body = format!(
                concat!(
                    "# HELP azums_queue_depth Number of queued jobs\n",
                    "# TYPE azums_queue_depth gauge\n",
                    "azums_queue_depth {}\n",
                    "# HELP azums_running_jobs Number of running jobs\n",
                    "# TYPE azums_running_jobs gauge\n",
                    "azums_running_jobs {}\n",
                    "# HELP azums_jobs_succeeded_last_60s Jobs succeeded in last 60s\n",
                    "# TYPE azums_jobs_succeeded_last_60s gauge\n",
                    "azums_jobs_succeeded_last_60s {}\n",
                    "# HELP azums_jobs_failed_last_60s Jobs failed/dlq in last 60s\n",
                    "# TYPE azums_jobs_failed_last_60s gauge\n",
                    "azums_jobs_failed_last_60s {}\n"
                ),
